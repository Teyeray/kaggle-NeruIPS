...
def get_data_paths():
    ...
    # 主数据目录
    BASE_PATH = os.getenv(
        "NEURIPS_DATA_PATH", "kaggle/input/neurips-open-polymer-prediction-2025"
    )
    ...
    paths = {
        ...
        # 外部数据文件
        "tg_jcim_data": os.getenv(
            "TG_JCIM_PATH", os.path.join(EXTRA_BASE, "JCIM_sup_bigsmiles.csv")
        ),
        "tg_excel_data": os.getenv(
            "TG_EXCEL_PATH", os.path.join(EXTRA_BASE, "data_tg3.xlsx")
        ),
        "density_data": os.getenv(
            "DENSITY_PATH", os.path.join(EXTRA_BASE, "data_dnst1.xlsx")
        ),
        # 补充数据目录
        "supplement_dir": os.getenv(
            "SUPPLEMENT_DIR", os.path.join(BASE_PATH, "train_supplement")
        ),
        "ffv_data": os.getenv(
            "FFV_DATA_PATH", os.path.join(BASE_PATH, "train_supplement", "dataset4.csv")
        ),
        # 其他补充数据
        "dataset1": os.getenv(
            "DATASET1_PATH", os.path.join(BASE_PATH, "train_supplement", "dataset1.csv")
        ),
        "dataset2": os.getenv(
            "DATASET2_PATH", os.path.join(BASE_PATH, "train_supplement", "dataset2.csv")
        ),
        "dataset3": os.getenv(
            "DATASET3_PATH", os.path.join(BASE_PATH, "train_supplement", "dataset3.csv")
        ),
    }
...
def load_and_split_data(
    paths: dict = None, test_size: float = 0.2, random_state: int = 42
):
    """
    1. 加载主训练集 train.csv
    2. 标准化 SMILES
    ...
    if os.path.exists(paths["tc_data"]):
        train = add_extra_data(
            train, pd.read_csv(paths["tc_data"]).rename(columns={"TC_mean": "Tc"}), "Tc"
        )
    if os.path.exists(paths["tg_jcim_data"]):
        train = add_extra_data(
            train,
            pd.read_csv(paths["tg_jcim_data"], usecols=["SMILES", "Tg (C)"]).rename(
                columns={"Tg (C)": "Tg"}
            ),
            "Tg",
        )
    if os.path.exists(paths["tg_excel_data"]):
        train = add_extra_data(
            train,
            pd.read_excel(paths["tg_excel_data"])
            .rename(columns={"Tg [K]": "Tg"})
            .assign(Tg=lambda df: df["Tg"] - 273.15),
            "Tg",
        )
    ...
    if os.path.exists(paths["ffv_data"]):
        train = add_extra_data(
            train, pd.read_csv(paths["ffv_data"]).rename(columns={"FFV": "FFV"}), "FFV"
        )
...
def create_graph_from_smiles(smiles):
    ...
    if x.size(0) != n_atoms:
        raise AssertionError(
            f"节点数量不匹配 ({x.size(0)}) vs 原子数 ({n_atoms}): {smiles}"
        )
    ...
    if edge_attr.size(0) != 2 * n_bonds:
        raise AssertionError(
            f"边数量不匹配 ({edge_attr.size(0)}) vs 2×键数 ({2*n_bonds}): {smiles}"
        )
...
def validate_graph(smiles, x, edge_index, edge_attr):
    ...
    if edge_attr.size(0) != 2 * n_bonds:
        raise AssertionError(
            f"Edges ({edge_attr.size(0)}) ≠ 2×bonds ({2*n_bonds}) for {smiles}"
        )
...
if __name__ == "__main__":
    ...
    print(
        f"🔢 样本总计 train={len(train_dataset)}, val={len(val_dataset)}, test={len(test_dataset)}"
    )
    ...
    # 6) 验证 shape
    assert (
        node_pred.shape == batch.x_orig.shape
    ), f"node_pred {node_pred.shape} vs x_orig {batch.x_orig.shape}"
    assert (
        edge_pred.shape == batch.edge_attr_orig.shape
